				...
				<el-col :span="24">
					<el-form-item label="维修账号" prop="weixiuzhanghao">
						<el-select
							...
							<el-option v-for="(item,index) in weixiuzhanghaoLists" :label="item" :value="item">
							</el-option>
						</el-select>
					</el-form-item>
				</el-col>
...
<script setup>
	...
	const init = (formId=null,formType='add',formNames='',row=null,table=null,statusColumnName=null,tips=null,statusColumnValue=null) => {
		...
		if(formType == 'add'){
			...
			for(let x in row){
				...
				if(x=='fenpeishijian'){
					form.value.fenpeishijian = row[x];
					disabledForm.value.fenpeishijian = true;
					continue;
				}
				if(x=='weixiuzhanghao'){
		...
		context?.$http({
			...
			weixiuzhanghaoLists.value = res.data.data
		})
		//由上级字段带出不可改
		disabledForm.value.weixiuxingming = true;
	...
	//维修账号回调
	const weixiuzhanghaoChange=()=>{
		context?.$http({
			url: `follow/weixiuyuan/weixiuzhanghao?columnValue=` + form.value.weixiuzhanghao,
			method: 'get'
		}).then(res=>{
			//带出维修姓名字段
			if(res.data.data.weixiuxingming){
				form.value.weixiuxingming = res.data.data.weixiuxingming
			}
		})
	}
	//提交
