	...
	<div class="detail-page" :style='{}'>
		...
		<div class="detail_view">
			...
			<div class="info_view">
				...
				<div class="info_item">
					<div class="info_label">分配时间</div>
					<div  class="info_text" >{{detail.fenpeishijian}}</div>
				</div>
				<div class="info_item">
				...
				<div class="btn_view">
					<el-button class="cross_btn" v-if="btnFrontAuth('weixiufenpei','维修')" @click="weixiuxinxionAcross('维修','','','weixiuzhuangtai','已维修','已维修,未维修'.split(',')[0])" type="warning">维修</el-button>
					<el-button class="edit_btn" v-if="centerType&&btnAuth('weixiufenpei','修改')" type="primary" @click="editClick">修改</el-button>
					<el-button class="del_btn" v-if="centerType&&btnAuth('weixiufenpei','删除')" type="danger" @click="delClick">删除</el-button>
				</div>
			</div>
		</div>
...
<script setup>
	...
	const weixiuxinxionAcross = async (btnType,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue,type=1) => {
		if(!context?.$toolUtil.storageGet('frontToken')){
			context?.$toolUtil.message('请登录后再操作！','error')
			return false
		}
		if(!btnAuth('weixiufenpei',btnType)){
		...
		nextTick(()=>{
			router.push(`/index/weixiuxinxiAdd?type=cross&&id=${detail.value.id}`)
		})
	}
	onMounted(()=>{
		init()
