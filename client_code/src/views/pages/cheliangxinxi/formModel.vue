	...
	<div class="detail-page" :style='{}'>
		...
		<div class="detail_view">
			...
			<div class="info_view">
				...
				<div class="info_item">
					<div class="info_label">车身颜色</div>
					<div  class="info_text" >{{detail.cheshenyanse}}</div>
				</div>
				<div class="info_item">
				...
				<div class="btn_view">
					<el-button class="cross_btn" v-if="btnFrontAuth('cheliangxinxi','预约')" @click="weixiuyuyueonAcross('预约','','','','')" type="warning">预约</el-button>
...
<script setup>
	...
	const weixiuyuyueonAcross = async (btnType,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue,type=1) => {
		if(!context?.$toolUtil.storageGet('frontToken')){
			context?.$toolUtil.message('请登录后再操作！','error')
			return false
		}
		if(!btnAuth('cheliangxinxi',btnType)){
		...
		nextTick(()=>{
			router.push(`/index/weixiuyuyueAdd?type=cross&&id=${detail.value.id}`)
		})
	}
	onMounted(()=>{
		init()
